                 ...
                 GLenum gl_target, GLenum gl_filter, GLenum iformat)
{
    assert(!fbo->fbo);
    assert(!fbo->texture);
    return fbotex_change(fbo, gl, log, w, h, gl_target, gl_filter, iformat, 0);
}
...
// As a special service, changing gl_filter does not require realloc, but
// simply changes the texture's filtering mode.
                   ...
                   int flags)
{
    ...
    if (fbo->fbo && fbo->tex_w == cw && fbo->tex_h == ch &&
        fbo->target == gl_target && fbo->iformat == iformat)
    {
        if (gl_filter != fbo->filter) {
